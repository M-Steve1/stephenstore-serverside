...
export const addProductsToCart = async (
  ...
  try {
    ...
    const isProductInCart = await cartService.isProductInCart(
      parseInt(cart_id),
      parseInt(product_id)
    );
    if (isProductInCart === null) {
      const addedProduct = await cartStore.addProductsToCart({
        cart_id,
        product_id,
        product_quantity
      });
      ...
      const updatedProduct = await cartStore.updateProductQuantity(
        parseInt(isProductInCart.id as unknown as string),
        parseInt(isProductInCart.product_quantity as unknown as string) +
