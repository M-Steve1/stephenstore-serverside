...
export const isProductInCart = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
    const { cart_id, product_id } = req.body;
    const result = await cartServiceStore.isProductInCart(
      parseInt(cart_id),
      parseInt(product_id)
    );
...
export const countProductsInCart = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
    const cart_id = req.params.id;
    const result = await cartServiceStore.countProductsInCart(
      parseInt(cart_id)
    );
    res.status(200).json(result);
  } catch (error) {
